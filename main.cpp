...
void text()
{
	...
	if (xx1)
	{
		...
		output(-0.190, 0.400, "International Islamic University Chittagong");
		output(-0.190, 0.325, "Dept. of Computer Science and Engineering");
		output(-0.010, 0.250, "MINI PROJECT ON");
		//output(0.0, 0.175, "ON");
		output(-0.075, 0.100, "3D House Interior Design");
		output(0.10, 0.025, "by");
		output(-0.040, -0.100, "Tasmi Khair Tapu");
		output(0.040, -0.155, "C211218");
		output(-0.020, -0.300, "Supervised by");
		output(-0.110, -0.380, "Umme Habiba Bushra");
		output(-0.040, -0.450, "Adjunct Lecturer");
...
void keys(unsigned char key, int x, int y)
{
	...
	if (key == 'Z')viewer[2] += 0.1;
	if (key == 'o') {
        sndPlaySound("Door.wav", SND_ASYNC); // Play the sound asynchronously
        delay(500); // Introduce a delay of 1000 milliseconds (1 second)
        doort(0); // Call doort function after the delay
    }
    if(key=='f'){

        sndPlaySound("Footstep.wav", SND_ASYNC);
    ...
    if(key=='c'){
        sndPlaySound("cook.wav", SND_ASYNC);
    }
